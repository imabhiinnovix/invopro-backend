/* eslint-disable @typescript-eslint/no-explicit-any */
/* @ts-nocheck */

import Permission from '../database/models/common/permissionModel'; // Your Mongoose model
import { getDynamicPermission } from '../database/services/common/permission.service';
interface PermissionSeed {
  name: string;
  method: string;
  resourceId: string;
  extraOptions?: Record<string, any>;
  resourceType: string;
  resourceCode: string;
  isChangeable: boolean;
  isSuperUser?: boolean;
  organizationId?: string; // 👈 added optional field
  module?: string;
  subModule?: string;
}

let permissions: PermissionSeed[] = [
  {
    name: 'List User',
    method: 'GET',
    resourceId: '/common/user/list',
    extraOptions: {},
    resourceType: 'User',
    resourceCode: 'user__list',
    isChangeable: true,
    module: "Setting",
    subModule: "User"
  },
  {
    name: 'Get Current User',
    method: 'GET',
    resourceId: '/common/user/get-current-user',
    extraOptions: {},
    resourceType: 'User',
    isSuperUser: false,
    isChangeable: false,
    resourceCode: 'user__get_current_user',
    module: "Setting",
    subModule: "User"
  },
  {
    name: 'Get User',
    method: 'GET',
    resourceId: '/common/user/:userId',
    resourceType: 'User',
    isSuperUser: false,
    isChangeable: false,
    resourceCode: 'user__get_user',
    module: "Setting",
    subModule: "User"
  },
  {
    name: 'Create User',
    method: 'POST',
    resourceId: '/common/user/create',
    extraOptions: {},
    resourceType: 'User',
    resourceCode: 'user__create',
    isSuperUser: false,
    isChangeable: true,
    module: "Setting",
    subModule: "User"
  },
  {
    name: 'Update Current User',
    method: 'PUT',
    resourceId: '/common/user/update-current-user',
    extraOptions: {},
    resourceType: 'User',
    resourceCode: 'user__update_current_user',
    isSuperUser: false,
    isChangeable: true,
    module: "Setting",
    subModule: "User Profile"
  },
  {
    name: 'Update Current User Password',
    method: 'PUT',
    resourceId: '/common/user/change-password',
    extraOptions: {},
    resourceType: 'User',
    resourceCode: 'user__change_password',
    isSuperUser: false,
    isChangeable: true,
    module: "Setting",
    subModule: "User Profile"
  },
  {
    name: 'Update User',
    method: 'PUT',
    resourceId: '/common/user/update/:userId',
    extraOptions: {},
    resourceType: 'User',
    resourceCode: 'user__update',
    isSuperUser: false,
    isChangeable: true,
    module: "Setting",
    subModule: "User"
  },
  {
    name: 'Delete User',
    method: 'DELETE',
    resourceId: '/common/user/delete/:userId',
    extraOptions: {},
    resourceType: 'User',
    resourceCode: 'user__delete',
    isSuperUser: false,
    isChangeable: true,
    module: "Setting",
    subModule: "User"
  },
  {
    name: 'List Permission',
    method: 'GET',
    resourceId: '/common/permission/list',
    extraOptions: {},
    resourceType: 'Permission',
    resourceCode: 'permission__list',
    isSuperUser: true,
    isChangeable: true,
    module: "Setting",
    subModule: "Permission"
  },
  {
    name: 'Create Permission',
    method: 'POST',
    resourceId: '/common/permission/create',
    extraOptions: {},
    resourceType: 'Permission',
    resourceCode: 'permission__create',
    isSuperUser: true,
    isChangeable: true,
    module: "Setting",
    subModule: "Permission"
  },
  {
    name: 'Update Permission',
    method: 'PUT',
    resourceId: '/common/permission/update/:permissionId',
    extraOptions: {},
    resourceType: 'Permission',
    resourceCode: 'permission__update',
    isSuperUser: true,
    isChangeable: true,
    module: "Setting",
    subModule: "Permission"
  },
  {
    name: 'Delete Permission',
    method: 'DELETE',
    resourceId: '/common/permission/delete/:permissionId',
    extraOptions: {},
    resourceType: 'Permission',
    resourceCode: 'permission__delete',
    isSuperUser: true,
    isChangeable: true,
    module: "Setting",
    subModule: "Permission"
  },
  {
    name: 'List Role',
    method: 'GET',
    resourceId: '/common/role/list',
    extraOptions: {},
    resourceType: 'Role',
    resourceCode: 'role__list',
    isSuperUser: false,
    isChangeable: true,
    module: "Setting",
    subModule: "Role"
  },
  {
    name: 'List Role Permission',
    method: 'GET',
    resourceId: '/common/role/:roleId',
    extraOptions: {},
    resourceType: 'Role',
    resourceCode: 'role__permission_list',
    isSuperUser: false,
    isChangeable: false,
    module: "Setting",
    subModule: "Role"
  },
  {
    name: 'Create Role',
    method: 'POST',
    resourceId: '/common/role/create',
    extraOptions: {},
    resourceType: 'Role',
    resourceCode: 'role__create',
    isSuperUser: false,
    isChangeable: true,
    module: "Setting",
    subModule: "Role"
  },
  {
    name: 'Update Role',
    method: 'PUT',
    resourceId: '/common/role/update/:roleId',
    extraOptions: {},
    resourceType: 'Role',
    resourceCode: 'role__update',
    isSuperUser: false,
    isChangeable: true,
    module: "Setting",
    subModule: "Role"
  },
  {
    name: 'Delete Role',
    method: 'DELETE',
    resourceId: '/common/role/delete/:roleId',
    extraOptions: {},
    resourceType: 'Role',
    resourceCode: 'role__delete',
    isSuperUser: false,
    isChangeable: true,
    module: "Setting",
    subModule: "Role"
  },
  {
    name: 'Product List',
    method: 'GET',
    resourceId: '/common/product/list',
    extraOptions: {},
    resourceType: 'Product',
    resourceCode: 'product__list',
    isSuperUser: false,
    isChangeable: false,
    module: "Setting",
    subModule: "Organization",
  },
  {
    name: 'Create Organization',
    method: 'POST',
    resourceId: '/common/organization/create',
    extraOptions: {},
    resourceType: 'Organization',
    resourceCode: 'organization__create',
    isSuperUser: true,
    isChangeable: true,
    module: "Setting",
    subModule: "Organization",
  },
  {
    name: 'Update Organization',
    method: 'PUT',
    resourceId: '/common/organization/update/:organizationId',
    extraOptions: {},
    resourceType: 'Organization',
    resourceCode: 'organization__update',
    isSuperUser: false,
    isChangeable: true,
    module: "Setting",
    subModule: "Organization",
  },
  {
    name: 'Delete Organization',
    method: 'DELETE',
    resourceId: '/common/organization/delete/:organizationId',
    extraOptions: {},
    resourceType: 'Organization',
    resourceCode: 'organization__delete',
    isSuperUser: true,
    isChangeable: true,
    module: "Setting",
    subModule: "Organization",
  },
  {
    name: 'Organization List',
    method: 'GET',
    resourceId: '/common/organization/list',
    extraOptions: {},
    resourceType: 'Organization',
    resourceCode: 'organization__list',
    isSuperUser: false,
    isChangeable: true,
    module: "Setting",
    subModule: "Organization",
  },
  {
    name: 'Get Organization',
    method: 'GET',
    resourceId: '/common/organization/:organizationId',
    extraOptions: {},
    resourceType: 'Organization',
    resourceCode: 'organization__get',
    isSuperUser: false,
    isChangeable: false,
    module: "Setting",
    subModule: "Organization",
  },
  {
    name: 'Get Current Organization',
    method: 'GET',
    resourceId: '/common/organization/get-current-organization',
    extraOptions: {},
    resourceType: 'Organization',
    resourceCode: 'organization__get_current',
    isSuperUser: false,
    isChangeable: false,
    module: "Setting",
    subModule: "Organization",
  },
  {
    name: 'Organization Product Subscription List',
    method: 'GET',
    resourceId: '/common/product-subscription/list',
    extraOptions: {},
    resourceType: 'Product Subscription',
    resourceCode: 'productSubscription__list',
    isSuperUser: false,
    isChangeable: false,
    module: "Setting",
    subModule: "Organization",
  },
  {
    name: 'Entities List',
    method: 'GET',
    resourceId: '/common/entities/list',
    extraOptions: {},
    resourceType: 'Entities',
    resourceCode: 'entities__list',
    isSuperUser: false,
    isChangeable: true,
    module: "Setting",
    subModule: "Entities",
  },
  {
    name: 'Get Entity Details',
    method: 'GET',
    resourceId: '/common/entities/:entityId',
    extraOptions: {},
    resourceType: 'Entities',
    resourceCode: 'entities__get',
    isSuperUser: false,
    isChangeable: false,
    module: "Setting",
    subModule: "Entities",
  },
  {
    name: 'Update Entity Details',
    method: 'PUT',
    resourceId: '/common/entities/update/:entityId',
    extraOptions: {},
    resourceType: 'Entities',
    resourceCode: 'entities__update',
    isSuperUser: false,
    isChangeable: true,
    module: "Setting",
    subModule: "Entities",
  },
  {
    name: 'Create Entity',
    method: 'POST',
    resourceId: '/common/entities/create',
    extraOptions: {},
    resourceType: 'Entities',
    resourceCode: 'entities__create',
    isSuperUser: false,
    isChangeable: true,
    module: "Setting",
    subModule: "Entities",
  },
  {
    name: 'Check Data Source Code Availability',
    method: 'GET',
    resourceId: '/common/dataSource/code/:code',
    extraOptions: {},
    resourceType: 'Data Source',
    resourceCode: 'dataSource__check_code',
    isSuperUser: false,
    isChangeable: false,
    module: "Setting",
    subModule: "Data Source",
  },
  {
    name: 'Check Data Source Name Availability',
    method: 'GET',
    resourceId: '/common/dataSource/name/:name',
    extraOptions: {},
    resourceType: 'Data Source',
    resourceCode: 'dataSource__check_name',
    isSuperUser: false,
    isChangeable: false,
    module: "Setting",
    subModule: "Data Source",
  },
  {
    name: 'Create Data Source',
    method: 'POST',
    resourceId: '/common/dataSource/create',
    extraOptions: {},
    resourceType: 'Data Source',
    resourceCode: 'dataSource__create',
    isSuperUser: false,
    isChangeable: true,
    module: "Setting",
    subModule: "Data Source",
  },
  {
    name: 'Update Data Source',
    method: 'PUT',
    resourceId: '/common/dataSource/update/:dataSourceId',
    extraOptions: {},
    resourceType: 'Data Source',
    resourceCode: 'dataSource__update',
    isSuperUser: false,
    isChangeable: true,
    module: "Setting",
    subModule: "Data Source",
  },
  {
    name: 'List Data Sources',
    method: 'GET',
    resourceId: '/common/dataSource/list',
    extraOptions: {},
    resourceType: 'Data Source',
    resourceCode: 'dataSource__list',
    isSuperUser: false,
    isChangeable: true,
    module: "Setting",
    subModule: "Data Source",
  },
  {
    name: 'Get Data Source By ID',
    method: 'GET',
    resourceId: '/common/dataSource/dataSourceId/:dataSourceId',
    extraOptions: {},
    resourceType: 'Data Source',
    resourceCode: 'dataSource__get',
    isSuperUser: false,
    isChangeable: false,
    module: "Setting",
    subModule: "Data Source",
  },
  {
    name: 'Get Widget Data By Filter',
    method: 'POST',
    resourceId: '/common/dataSource/getWidgetDataByFilter',
    extraOptions: {},
    resourceType: 'Data Source',
    resourceCode: 'dataSource__get_widget_data',
    isSuperUser: false,
    isChangeable: false,
    module: "Dashboard",
    subModule: "Custom Dashboard",
  },
  {
    name: 'Export Widget Data By Filter',
    method: 'POST',
    resourceId: '/common/dataSource/getWidgetDataByFilter/export',
    extraOptions: {},
    resourceType: 'Data Source',
    resourceCode: 'dataSource__export_widget_data',
    isSuperUser: false,
    isChangeable: true,
    module: "Dashboard",
    subModule: "Custom Dashboard",
  },
  {
    name: 'Create Attribute Option',
    method: 'POST',
    resourceId: '/common/attributeOptions/create',
    extraOptions: {},
    resourceType: 'Attribute Option',
    resourceCode: 'attributeOption__create',
    isSuperUser: false,
    isChangeable: false,
    module: "Setting",
    subModule: "Attribute Option",
  },
  {
    name: 'Update Attribute Option',
    method: 'PUT',
    resourceId: '/common/attributeOptions/update/:attributeId',
    extraOptions: {},
    resourceType: 'Attribute Option',
    resourceCode: 'attributeOption__update',
    isSuperUser: false,
    isChangeable: false,
    module: "Setting",
    subModule: "Attribute Option",
  },
  {
    name: 'List Attribute Options',
    method: 'GET',
    resourceId: '/common/attributeOptions/list',
    extraOptions: {},
    resourceType: 'Attribute Option',
    resourceCode: 'attributeOption__list',
    isSuperUser: false,
    isChangeable: false,
    module: "Setting",
    subModule: "Attribute Option",
  },
  {
    name: 'Get Attribute Option By ID',
    method: 'GET',
    resourceId: '/common/attributeOptions/get/:attributeId',
    extraOptions: {},
    resourceType: 'Attribute Option',
    resourceCode: 'attributeOption__get',
    isSuperUser: false,
    isChangeable: false,
    module: "Setting",
    subModule: "Attribute Option",
  },
  {
    name: 'File Upload',
    method: 'POST',
    resourceId: '/common/files/upload',
    extraOptions: {},
    resourceType: 'File Upload',
    resourceCode: 'fileUpload__upload',
    isSuperUser: false,
    isChangeable: true,
    module: "Setting",
    subModule: "Data Source Version"
  },
  {
    name: 'List Data Source Versions',
    method: 'GET',
    resourceId: '/common/dataSourceVersion/list',
    extraOptions: {},
    resourceType: 'Data Source Version',
    resourceCode: 'dataSourceVersion__list',
    isSuperUser: false,
    isChangeable: true,
    module: "Setting",
    subModule: "Data Source Version"
  },
  {
    name: 'Check Data Source Version Name Availability',
    method: 'GET',
    resourceId:
      '/common/dataSourceVersion/dataSourceId/:dataSourceId/versionValue/:versionValue/versionName/:versionName',
    extraOptions: {},
    resourceType: 'Data Source Version',
    resourceCode: 'dataSourceVersion__check_name',
    isSuperUser: false,
    isChangeable: false,
    module: "Setting",
    subModule: "Data Source Version"
  },
  {
    name: 'Create or Update Data Source Version Value',
    method: 'POST',
    resourceId: '/common/dataSourceVersion/create',
    extraOptions: {},
    resourceType: 'Data Source Version',
    resourceCode: 'dataSourceVersion__create_update',
    isSuperUser: false,
    isChangeable: false,
    module: "Setting",
    subModule: "Data Source Version"
  },
  {
    name: 'List Data Source Version Data by Data Source and Version Value',
    method: 'GET',
    resourceId: '/common/dataSourceVersion/versionData',
    extraOptions: {},
    resourceType: 'Data Source Version',
    resourceCode: 'dataSourceVersion__get_data',
    isSuperUser: false,
    isChangeable: true,
    module: "Setting",
    subModule: "Data Source Version"
  },
  {
    name: 'Export Data Source Version Data by Data Source and Version Value',
    method: 'GET',
    resourceId: '/common/dataSourceVersion/versionData/export',
    extraOptions: {},
    resourceType: 'Data Source Version',
    resourceCode: 'dataSourceVersion__export_data',
    isSuperUser: false,
    isChangeable: true,
    module: "Setting",
    subModule: "Data Source Version"
  },
  {
    name: 'Create Data Source Version Value',
    method: 'POST',
    resourceId: '/common/dataSourceVersion/versionData/create',
    extraOptions: {},
    resourceType: 'Data Source Version',
    resourceCode: 'dataSourceVersion_create_data',
    isSuperUser: false,
    isChangeable: true,
    module: "Setting",
    subModule: "Data Source Version"
  },
  {
    name: 'Update Data Source Version Value',
    method: 'PUT',
    resourceId: '/common/dataSourceVersion/versionData/update/:rowId',
    extraOptions: {},
    resourceType: 'Data Source Version',
    resourceCode: 'dataSourceVersion_update_data',
    isSuperUser: false,
    isChangeable: true,
    module: "Setting",
    subModule: "Data Source Version"
  },
  {
    name: 'Delete Data Source Version Value',
    method: 'DELETE',
    resourceId: '/common/dataSourceVersion/versionData/delete',
    extraOptions: {},
    resourceType: 'Data Source Version',
    resourceCode: 'dataSourceVersion_delete_data',
    isSuperUser: false,
    isChangeable: true,
    module: "Setting",
    subModule: "Data Source Version"
  },
  {
    name: 'Get Latest Data Source Version Detail by Custom Report ID',
    method: 'GET',
    resourceId: '/common/dataSourceVersion/listVersionData/:customReportId',
    extraOptions: {},
    resourceType: 'Data Source Version',
    resourceCode: 'dataSourceVersion__get_latest_detail',
    isSuperUser: false,
    isChangeable: false,
    module: "Setting",
    subModule: "Data Source Version"
  },
  {
    name: 'List All Available Data Source Version Values',
    method: 'GET',
    resourceId: '/common/dataSourceVersion/listAllAvailableDataSourceVersionValue',
    extraOptions: {},
    resourceType: 'Data Source Version',
    resourceCode: 'dataSourceVersion__list_all_available',
    isSuperUser: false,
    isChangeable: false,
    module: "Setting",
    subModule: "Data Source Version"
  },
  {
    name: 'Get Chart Data',
    method: 'POST',
    resourceId: '/common/dataSourceVersion/chartData',
    extraOptions: {},
    resourceType: 'Data Source Version',
    resourceCode: 'dataSourceVersion__get_chart_data',
    isSuperUser: false,
    isChangeable: false,
    module: "Dashboard",
    subModule: "Data Source Version"
  },
  {
    name: 'List Data Import Errors',
    method: 'GET',
    resourceId: '/common/dataImportError/list',
    extraOptions: {},
    resourceType: 'Data Source Version',
    resourceCode: 'dataSourceVersion__list_import_errors',
    isSuperUser: false,
    isChangeable: false,
    module: "Setting",
    subModule: "Data Source Version"
  },
  {
    name: 'List Dashboards',
    method: 'GET',
    resourceId: '/common/dashboard/list',
    extraOptions: {},
    resourceType: 'Dashboard',
    resourceCode: 'dashboard__list',
    isSuperUser: false,
    isChangeable: true,
    module: "Dashboard",
    subModule: 'Custom Dashboard',
  },
  {
    name: 'Get Dashboard By ID',
    method: 'GET',
    resourceId: '/common/dashboard/get/:dashboardId',
    extraOptions: {},
    resourceType: 'Dashboard',
    resourceCode: 'dashboard__get',
    isSuperUser: false,
    isChangeable: false,
    module: "Dashboard",
    subModule: 'Custom Dashboard',
  },
  {
    name: 'Create Dashboard',
    method: 'POST',
    resourceId: '/common/dashboard/create',
    extraOptions: {},
    resourceType: 'Dashboard',
    resourceCode: 'dashboard__create',
    isSuperUser: false,
    isChangeable: true,
    module: "Dashboard",
    subModule: 'Custom Dashboard',
  },
  {
    name: 'Update Dashboard',
    method: 'POST',
    resourceId: '/common/dashboard/update/:dashboardId',
    extraOptions: {},
    resourceType: 'Dashboard',
    resourceCode: 'dashboard__update',
    isSuperUser: false,
    isChangeable: true,
    module: "Dashboard",
    subModule: 'Custom Dashboard',
  },
  {
    name: 'Delete Dashboard',
    method: 'POST',
    resourceId: '/common/dashboard/delete/:dashboardId',
    extraOptions: {},
    resourceType: 'Dashboard',
    resourceCode: 'dashboard__delete',
    isSuperUser: false,
    isChangeable: true,
    module: "Dashboard",
    subModule: 'Custom Dashboard',
  },
  {
    name: 'Select Dashboard Theme',
    method: 'POST',
    resourceId: '/common/dashboard/selectTheme/:dashboardId',
    extraOptions: {},
    resourceType: 'Dashboard',
    resourceCode: 'dashboard__select_theme',
    isSuperUser: false,
    isChangeable: false,
    module: "Dashboard",
    subModule: 'Theme',
  },
  {
    name: 'List Dashboard Widgets',
    method: 'GET',
    resourceId: '/common/dashboard/widget/getWidgets/:dashboardId',
    extraOptions: {},
    resourceType: 'Dashboard',
    resourceCode: 'dashboard__get_widgets',
    isSuperUser: false,
    isChangeable: true,
    module: "Dashboard",
    subModule: 'Widget',
  },
  {
    name: 'Get Widget Data',
    method: 'POST',
    resourceId: '/common/dashboard/widget/getWidgetData',
    extraOptions: {},
    resourceType: 'Dashboard',
    resourceCode: 'dashboard__get_widget_data',
    isSuperUser: false,
    isChangeable: false,
    module: "Dashboard",
    subModule: 'Widget',
  },
  {
    name: 'Get Widget Plot Types',
    method: 'GET',
    resourceId: '/common/dashboard/widget/getPlotTypes',
    extraOptions: {},
    resourceType: 'Dashboard',
    resourceCode: 'dashboard__get_widget_plot_types',
    isSuperUser: false,
    isChangeable: false,
    module: "Dashboard",
    subModule: 'Widget',
  },
  {
    name: 'Create Widget',
    method: 'POST',
    resourceId: '/common/dashboard/widget/create',
    extraOptions: {},
    resourceType: 'Dashboard',
    resourceCode: 'dashboard__create_widget',
    isSuperUser: false,
    isChangeable: true,
    module: "Dashboard",
    subModule: 'Widget',
  },
  {
    name: 'Save Dashboard Widgets',
    method: 'POST',
    resourceId: '/common/dashboard/widget/save',
    extraOptions: {},
    resourceType: 'Dashboard',
    resourceCode: 'dashboard__save_widgets',
    isSuperUser: false,
    isChangeable: true,
    module: "Dashboard",
    subModule: 'Widget',
  },
  {
    name: 'Update Widget',
    method: 'POST',
    resourceId: '/common/dashboard/widget/update/:dashboardWidgetId',
    extraOptions: {},
    resourceType: 'Dashboard',
    resourceCode: 'dashboard__update_widget',
    isSuperUser: false,
    isChangeable: true,
    module: "Dashboard",
    subModule: 'Widget',
  },
  {
    name: 'Delete Widget',
    method: 'POST',
    resourceId: '/common/dashboard/widget/delete/:dashboardWidgetId',
    extraOptions: {},
    resourceType: 'Dashboard',
    resourceCode: 'dashboard__delete_widget',
    isSuperUser: false,
    isChangeable: true,
    module: "Dashboard",
    subModule: 'Widget',
  },
  {
    name: 'Create Widget Type',
    method: 'POST',
    resourceId: '/common/widgetType/create',
    extraOptions: {},
    resourceType: 'Widget Type',
    resourceCode: 'widgetType__create',
    isSuperUser: true,
    isChangeable: false,
    module: "Dashboard",
    subModule: 'Widget Type',
  },
  {
    name: 'Update Widget Type',
    method: 'POST',
    resourceId: '/common/widgetType/update/:widgetTypeId',
    extraOptions: {},
    resourceType: 'Widget Type',
    resourceCode: 'widgetType__update',
    isSuperUser: true,
    isChangeable: false,
    module: "Dashboard",
    subModule: 'Widget Type',
  },
  {
    name: 'Delete Widget Type',
    method: 'POST',
    resourceId: '/common/widgetType/delete/:widgetTypeId',
    extraOptions: {},
    resourceType: 'Widget Type',
    resourceCode: 'widgetType__delete',
    isSuperUser: true,
    isChangeable: false,
    module: "Dashboard",
    subModule: 'Widget Type',
  },
  {
    name: 'Get Widget Type By ID',
    method: 'GET',
    resourceId: '/common/widgetType/get/:widgetTypeId',
    extraOptions: {},
    resourceType: 'Widget Type',
    resourceCode: 'widgetType__get',
    isSuperUser: false,
    isChangeable: false,
    module: "Dashboard",
    subModule: 'Widget Type',
  },
  {
    name: 'List Widget Types',
    method: 'GET',
    resourceId: '/common/widgetType/list',
    extraOptions: {},
    resourceType: 'Widget Type',
    resourceCode: 'widgetType__list',
    isSuperUser: false,
    isChangeable: false,
    module: "Dashboard",
    subModule: 'Widget Type',
  },
  {
    name: 'Get Operator By ID',
    method: 'GET',
    resourceId: '/common/operator/get/:operatorId',
    extraOptions: {},
    resourceType: 'Operator',
    resourceCode: 'operator__get',
    isSuperUser: false,
    isChangeable: false,
    module: "Setting",
    subModule: 'Operator',
  },
  {
    name: 'List Operators',
    method: 'POST',
    resourceId: '/common/operator/list',
    extraOptions: {},
    resourceType: 'Operator',
    resourceCode: 'operator__list',
    isSuperUser: false,
    isChangeable: false,
    module: "Setting",
    subModule: 'Operator',
  },
  {
    name: 'Update Operator',
    method: 'POST',
    resourceId: '/common/operator/update/:operatorId',
    extraOptions: {},
    resourceType: 'Operator',
    resourceCode: 'operator__update',
    isSuperUser: true,
    isChangeable: false,
    module: "Setting",
    subModule: 'Operator',
  },
  {
    name: 'Create Operator',
    method: 'POST',
    resourceId: '/common/operator/create',
    extraOptions: {},
    resourceType: 'Operator',
    resourceCode: 'operator__create',
    isSuperUser: true,
    isChangeable: false,
    module: "Setting",
    subModule: 'Operator',
  },
  {
    name: 'List Widget Themes',
    method: 'GET',
    resourceId: '/common/widgetTheme/list',
    extraOptions: {},
    resourceType: 'get_data_by_version_idWidget Theme',
    resourceCode: 'widgetTheme__list',
    isSuperUser: false,
    isChangeable: false,
    module: "Dashboard",
    subModule: "Widget Theme"
  },
  {
    name: 'Get Widget Theme By ID',
    method: 'GET',
    resourceId: '/common/widgetTheme/:widgetThemeId',
    extraOptions: {},
    resourceType: 'Widget Theme',
    resourceCode: 'widgetTheme__get',
    isSuperUser: false,
    isChangeable: false,
    module: "Dashboard",
    subModule: "Widget Theme"
  },
  {
    name: 'Create Widget Theme',
    method: 'POST',
    resourceId: '/common/widgetTheme/create',
    extraOptions: {},
    resourceType: 'Widget Theme',
    resourceCode: 'widgetTheme__create',
    isSuperUser: false,
    isChangeable: false,
    module: "Dashboard",
    subModule: "Widget Theme"
  },
  {
    name: 'Duplicate Widget Theme',
    method: 'POST',
    resourceId: '/common/widgetTheme/duplicate/:widgetThemeId',
    extraOptions: {},
    resourceType: 'Widget Theme',
    resourceCode: 'widgetTheme__duplicate',
    isSuperUser: false,
    isChangeable: false,
    module: "Dashboard",
    subModule: "Widget Theme"
  },
  {
    name: 'Update Widget Theme',
    method: 'POST',
    resourceId: '/common/widgetTheme/update/:widgetThemeId',
    extraOptions: {},
    resourceType: 'Widget Theme',
    resourceCode: 'widgetTheme__update',
    isSuperUser: false,
    isChangeable: false,
    module: "Dashboard",
    subModule: "Widget Theme"
  },
  {
    name: 'Delete Widget Theme',
    method: 'POST',
    resourceId: '/common/widgetTheme/delete/:widgetThemeId',
    extraOptions: {},
    resourceType: 'Widget Theme',
    resourceCode: 'widgetTheme__delete',
    isSuperUser: false,
    isChangeable: false,
    module: "Dashboard",
    subModule: "Widget Theme"
  },
  {
    name: 'List Dashboard Shares',
    method: 'GET',
    resourceId: '/common/dashboardShare/list/:dashboardId',
    extraOptions: {},
    resourceType: 'Dashboard Share',
    resourceCode: 'dashboardShare__list',
    isSuperUser: false,
    isChangeable: false,
    module: "Dashboard",
    subModule: "Dashboard Share"
  },
  {
    name: 'Create Dashboard Share',
    method: 'POST',
    resourceId: '/common/dashboardShare/create',
    extraOptions: {},
    resourceType: 'Dashboard Share',
    resourceCode: 'dashboardShare__create',
    isSuperUser: false,
    isChangeable: false,
    module: "Dashboard",
    subModule: "Dashboard Share"
  },
  {
    name: 'Delete Dashboard Share',
    method: 'POST',
    resourceId: '/common/dashboardShare/:dashboardShareId',
    extraOptions: {},
    resourceType: 'Dashboard Share',
    resourceCode: 'dashboardShare__delete',
    isSuperUser: false,
    isChangeable: false,
    module: "Dashboard",
    subModule: "Dashboard Share"
  },
  {
    name: 'List Custom Reports',
    method: 'GET',
    resourceId: '/reportivix/customReports/list',
    extraOptions: {},
    resourceType: 'Custom Report',
    resourceCode: 'customReport__list',
    isSuperUser: false,
    isChangeable: false,
    module: "Reports",
    subModule: "Custom Report"
  },
  {
    name: 'List Report Requests',
    method: 'GET',
    resourceId: '/reportivix/customReports/listReportRequest',
    extraOptions: {},
    resourceType: 'Custom Report',
    resourceCode: 'customReport__list_requests',
    isSuperUser: false,
    isChangeable: true,
    module: "Reports",
    subModule: "Custom Report"
  },
  {
    name: 'Get Report Version Values',
    method: 'GET',
    resourceId: '/reportivix/customReports/getVersionValue',
    extraOptions: {},
    resourceType: 'Custom Report',
    resourceCode: 'customReport__get_version_values',
    isSuperUser: false,
    isChangeable: false,
    module: "Reports",
    subModule: "Custom Report"
  },
  {
    name: 'Generate Custom Report',
    method: 'POST',
    resourceId: '/reportivix/customReports/generate',
    extraOptions: {},
    resourceType: 'Custom Report',
    resourceCode: 'customReport__generate',
    isSuperUser: false,
    isChangeable: true,
    module: "Reports",
    subModule: "Custom Report"
  },
  {
    name: 'Download Report',
    method: 'GET',
    resourceId: '/reportivix/customReports/downloadSupplementalIntermediate/:customReportId',
    extraOptions: {},
    resourceType: 'Custom Report',
    resourceCode: 'customReport__download_supplemental_intermediate',
    isSuperUser: false,
    isChangeable: false,
    module: "Reports",
    subModule: "Custom Report"
  },
  {
    name: 'Download Report',
    method: 'GET',
    resourceId: '/reportivix/customReports/download/:reportRequestId',
    extraOptions: {},
    resourceType: 'Custom Report',
    resourceCode: 'customReport__download',
    isSuperUser: false,
    isChangeable: true,
    module: "Reports",
    subModule: "Custom Report"
  },
  {
    name: 'Get Report Data by Data Source Version ID',
    method: 'GET',
    resourceId: '/reportivix/customReports/reportDataOnDataSourceVersionId/:dataSourceVersionId',
    extraOptions: {},
    resourceType: 'Custom Report',
    resourceCode: 'customReport__get_data_by_version_id',
    isSuperUser: false,
    isChangeable: false,
    module: "Reports",
    subModule: "Custom Report"
  },
  {
    name: 'Get Custom Report Design Details',
    method: 'GET',
    resourceId: '/reportivix/customReports/customReportDesignData/:customReportId',
    extraOptions: {},
    resourceType: 'Custom Report',
    resourceCode: 'customReport__get_design_details',
    isSuperUser: false,
    isChangeable: false,
    module: "Reports",
    subModule: "Custom Report"
  },
  {
    name: 'Get Report Request Details',
    method: 'GET',
    resourceId: '/reportivix/customReports/reportDetails/:reportRequestId',
    extraOptions: {},
    resourceType: 'Custom Report',
    resourceCode: 'customReport__get_request_details',
    isSuperUser: false,
    isChangeable: false,
    module: "Reports",
    subModule: "Custom Report"
  },
  {
    name: 'Get Custom Report Data by Data Source & Version Range',
    method: 'GET',
    resourceId: '/reportivix/customReports/reportData/:dataSourceId',
    extraOptions: {},
    resourceType: 'Custom Report',
    resourceCode: 'customReport__get_data_by_range',
    isSuperUser: false,
    isChangeable: false,
    module: "Reports",
    subModule: "Custom Report"
  },
  {
    name: 'List Custom Report Settings',
    method: 'GET',
    resourceId: '/reportivix/customReports/listSettings',
    extraOptions: {},
    resourceType: 'Custom Report',
    resourceCode: 'customReport__list_settings',
    isSuperUser: false,
    isChangeable: true,
    module: "Reports",
    subModule: "Custom Report Setting"
  },
  {
    name: 'Update Custom Report Settings',
    method: 'POST',
    resourceId: '/reportivix/customReports/updateSettings/:customReportId',
    extraOptions: {},
    resourceType: 'Custom Report',
    resourceCode: 'customReport__update_settings',
    isSuperUser: false,
    isChangeable: true,
    module: "Reports",
    subModule: "Custom Report Setting"
  },
  {
    name: 'Run Natural Language Query',
    method: 'GET',
    resourceId: '/reportivix/nlQuery/getData',
    extraOptions: {},
    resourceType: 'NL Query',
    resourceCode: 'nlQuery__get_data',
    isSuperUser: false,
    isChangeable: true,
    module: "VixAI",
    subModule: 'VixAI Query'
  },
  {
    name: 'Run Natural Language Insights',
    method: 'GET',
    resourceId: '/reportivix/nlQuery/insights',
    extraOptions: {},
    resourceType: 'NL Query',
    resourceCode: 'nlQuery__insights',
    isSuperUser: false,
    isChangeable: true,
    module: "VixAI",
    subModule: 'VixAI Query'
  },
  {
    name: 'Create Derived Field',
    method: 'POST',
    resourceId: '/common/derivedField/create',
    extraOptions: {},
    resourceType: 'Derived Field',
    resourceCode: 'derived_field__create',
    isSuperUser: false,
    isChangeable: false,
    module: "Setting",
    subModule: 'Derived Field'
  },
  {
    name: 'Update Derived Field',
    method: 'PUT',
    resourceId: '/common/derivedField/update/:id',
    extraOptions: {},
    resourceType: 'Derived Field',
    resourceCode: 'derived_field__update',
    isSuperUser: false,
    isChangeable: false,
    module: "Setting",
    subModule: 'Derived Field'
  },
  {
    name: 'Delete Derived Field',
    method: 'DELETE',
    resourceId: '/common/derivedField/delete/:id',
    extraOptions: {},
    resourceType: 'Derived Field',
    resourceCode: 'derived_field__delete',
    isSuperUser: false,
    isChangeable: false,
    module: "Setting",
    subModule: 'Derived Field'
  },
  {
    name: 'List Derived Fields',
    method: 'GET',
    resourceId: '/common/derivedField/list',
    extraOptions: {},
    resourceType: 'Derived Field',
    resourceCode: 'derived_field__list',
    isSuperUser: false,
    isChangeable: false,
    module: "Setting",
    subModule: 'Derived Field'
  },
  {
    name: 'Get Derived Field',
    method: 'GET',
    resourceId: '/common/derivedField/:id',
    extraOptions: {},
    resourceType: 'Derived Field',
    resourceCode: 'derived_field__get',
    isSuperUser: false,
    isChangeable: false,
    module: "Setting",
    subModule: 'Derived Field'
  },
  {
    name: 'List Notification Types',
    method: 'GET',
    resourceId: '/notivix/notification-setting/type/list',
    extraOptions: {},
    resourceType: 'Notification Setting Type',
    resourceCode: 'notification_setting_type__list',
    isSuperUser: false,
    isChangeable: true,
    module: "Notifications",
    subModule: "Alert Setting"
  },
  {
    name: 'Create Notification Type',
    method: 'POST',
    resourceId: '/notivix/notification-setting/type/create',
    extraOptions: {},
    resourceType: 'Notification Setting Type',
    resourceCode: 'notification_setting_type__create',
    isSuperUser: false,
    isChangeable: true,
    module: "Notifications",
    subModule: "Alert Setting"
  },
  {
    name: 'Update Notification Type',
    method: 'PUT',
    resourceId: '/notivix/notification-setting/type/update/:id',
    extraOptions: {},
    resourceType: 'Notification Setting Type',
    resourceCode: 'notification_setting_type__update',
    isSuperUser: false,
    isChangeable: true,
    module: "Notifications",
    subModule: "Alert Setting"
  },
  {
    name: 'Delete Notification Type',
    method: 'DELETE',
    resourceId: '/notivix/notification-setting/type/delete/:id',
    extraOptions: {},
    resourceType: 'Notification Setting Type',
    resourceCode: 'notification_setting_type__delete',
    isSuperUser: false,
    isChangeable: true,
    module: "Notifications",
    subModule: "Alert Setting"
  },
  {
    name: 'Summary Notification Type',
    method: 'POST',
    resourceId: '/notivix/notification-setting/type/summary',
    extraOptions: {},
    resourceType: 'Notification Setting Type',
    resourceCode: 'notification_setting_type__summary_create',
    isSuperUser: false,
    isChangeable: false,
    module: "Notifications",
    subModule: "Alert Setting"
  },
  {
    name: 'Get Notification Type',
    method: 'GET',
    resourceId: '/notivix/notification-setting/type/:id',
    extraOptions: {},
    resourceType: 'Notification Setting Type',
    resourceCode: 'notification_setting_type__get',
    isSuperUser: false,
    isChangeable: false,
    module: "Notifications",
    subModule: "Alert Setting"
  },
  {
    name: 'List Notification Frequencies',
    method: 'GET',
    resourceId: '/notivix/notification-setting/frequency/list',
    extraOptions: {},
    resourceType: 'Notification Setting Frequency',
    resourceCode: 'notification_setting_frequency__list',
    isSuperUser: false,
    isChangeable: true,
    module: "Notifications",
    subModule: "Scheduler"
  },
  {
    name: 'Create Notification Frequency',
    method: 'POST',
    resourceId: '/notivix/notification-setting/frequency/create',
    extraOptions: {},
    resourceType: 'Notification Setting Frequency',
    resourceCode: 'notification_setting_frequency__create',
    isSuperUser: false,
    isChangeable: true,
    module: "Notifications",
    subModule: "Scheduler"
  },
  {
    name: 'Update Notification Frequency',
    method: 'PUT',
    resourceId: '/notivix/notification-setting/frequency/update/:id',
    extraOptions: {},
    resourceType: 'Notification Setting Frequency',
    resourceCode: 'notification_setting_frequency__update',
    isSuperUser: false,
    isChangeable: true,
    module: "Notifications",
    subModule: "Scheduler"
  },
  {
    name: 'Delete Notification Frequency',
    method: 'DELETE',
    resourceId: '/notivix/notification-setting/frequency/delete/:id',
    extraOptions: {},
    resourceType: 'Notification Setting Frequency',
    resourceCode: 'notification_setting_frequency__delete',
    isSuperUser: false,
    isChangeable: true,
    module: "Notifications",
    subModule: "Scheduler"
  },
  {
    name: 'Get Notification Frequency',
    method: 'GET',
    resourceId: '/notivix/notification-setting/frequency/:id',
    extraOptions: {},
    resourceType: 'Notification Setting Frequency',
    resourceCode: 'notification_setting_frequency__get',
    isSuperUser: false,
    isChangeable: false,
    module: "Notifications",
    subModule: "Scheduler"
  },
  {
    name: 'List Notification Templates',
    method: 'GET',
    resourceId: '/notivix/notification-setting/template/list',
    extraOptions: {},
    resourceType: 'Notification Setting Template',
    resourceCode: 'notification_setting_template__list',
    isSuperUser: false,
    isChangeable: true,
    module: "Notifications",
    subModule: "Template"
  },
  {
    name: 'Create Notification Template',
    method: 'POST',
    resourceId: '/notivix/notification-setting/template/create',
    extraOptions: {},
    resourceType: 'Notification Setting Template',
    resourceCode: 'notification_setting_template__create',
    isSuperUser: false,
    isChangeable: true,
    module: "Notifications",
    subModule: "Template"
  },
  {
    name: 'Update Notification Template',
    method: 'PUT',
    resourceId: '/notivix/notification-setting/template/update/:id',
    extraOptions: {},
    resourceType: 'Notification Setting Template',
    resourceCode: 'notification_setting_template__update',
    isSuperUser: false,
    isChangeable: true,
    module: "Notifications",
    subModule: "Template"
  },
  {
    name: 'Delete Notification Template',
    method: 'DELETE',
    resourceId: '/notivix/notification-setting/template/delete/:id',
    extraOptions: {},
    resourceType: 'Notification Setting Template',
    resourceCode: 'notification_setting_template__delete',
    isSuperUser: false,
    isChangeable: true,
    module: "Notifications",
    subModule: "Template"
  },
  {
    name: 'Get Notification Template',
    method: 'GET',
    resourceId: '/notivix/notification-setting/template/:id',
    extraOptions: {},
    resourceType: 'Notification Setting Template',
    resourceCode: 'notification_setting_template__get',
    isSuperUser: false,
    isChangeable: false,
    module: "Notifications",
    subModule: "Template"
  },
  {
    name: 'List Notification Medium Settings',
    method: 'GET',
    resourceId: '/notivix/notification-setting/medium/list',
    extraOptions: {},
    resourceType: 'Notification Medium Setting',
    resourceCode: 'notification_setting_medium__list',
    isSuperUser: false,
    isChangeable: true,
    module: "Setting",
    subModule: "Notification Medium"
    
  },
  {
    name: 'Create Notification Medium Setting',
    method: 'POST',
    resourceId: '/notivix/notification-setting/medium/create',
    extraOptions: {},
    resourceType: 'Notification Medium Setting',
    resourceCode: 'notification_setting_medium__create',
    isSuperUser: false,
    isChangeable: true,
    module: "Setting",
    subModule: "Notification Medium"
  },
  {
    name: 'Update Notification Medium Setting',
    method: 'PUT',
    resourceId: '/notivix/notification-setting/medium/update/:id',
    extraOptions: {},
    resourceType: 'Notification Medium Setting',
    resourceCode: 'notification_setting_medium__update',
    isSuperUser: false,
    isChangeable: true,
    module: "Setting",
    subModule: "Notification Medium"
  },
  {
    name: 'Delete Notification Medium Setting',
    method: 'DELETE',
    resourceId: '/notivix/notification-setting/medium/delete',
    extraOptions: {},
    resourceType: 'Notification Medium Setting',
    resourceCode: 'notification_setting_medium__delete',
    isSuperUser: false,
    isChangeable: true,
    module: "Setting",
    subModule: "Notification Medium"
  },
  {
    name: 'Get Notification Medium',
    method: 'GET',
    resourceId: '/notivix/notification-setting/medium/:id',
    extraOptions: {},
    resourceType: 'Notification Medium Setting',
    resourceCode: 'notification_setting_medium__get',
    isSuperUser: false,
    isChangeable: false,
    module: "Setting",
    subModule: "Notification Medium"
  },
  {
    name: 'Trigger Notification Prepared',
    method: 'POST',
    resourceId: '/notivix/notification-setting/prepared/trigger',
    extraOptions: {},
    resourceType: 'Notification Prepared',
    resourceCode: 'notification_prepared_trigger__create',
    isSuperUser: false,
    isChangeable: true,
    module: "Notifications",
    subModule: "Notification Logs"
  },
  {
    name: 'List Notification Prepared',
    method: 'GET',
    resourceId: '/notivix/notification-setting/prepared/notification/list',
    extraOptions: {},
    resourceType: 'Notification Prepared',
    resourceCode: 'notification_prepared__list',
    isSuperUser: false,
    isChangeable: true,
    module: "Notifications",
    subModule: "Notification Logs"
  },
  {
    name: 'Resend Notification Prepared',
    method: 'POST',
    resourceId: '/notivix/notification-setting/prepared/notification/resend',
    extraOptions: {},
    resourceType: 'Notification Prepared',
    resourceCode: 'notification_prepared_resend__create',
    isSuperUser: false,
    isChangeable: true,
    module: "Notifications",
    subModule: "Notification Logs"
  },
  {
    name: 'List Dashboard Font Themes',
    method: 'GET',
    resourceId: '/common/dashboardFont/list',
    extraOptions: {},
    resourceType: 'Dashboard Font Theme',
    resourceCode: 'dashboard_font_theme__list',
    isSuperUser: false,
    isChangeable: false,
    module: "Dashboard",
    subModule: "Font Theme"
  },
  {
    name: 'Download Dashboard Font Theme',
    method: 'GET',
    resourceId: '/common/dashboardFont/download/:fontId',
    extraOptions: {},
    resourceType: 'Dashboard Font Theme',
    resourceCode: 'dashboard_font_theme__download',
    isSuperUser: false,
    isChangeable: false,
    module: "Dashboard",
    subModule: "Font Theme"
  },
  {
    name: 'Delete Dashboard Font Theme',
    method: 'DELETE',
    resourceId: '/common/dashboardFont/delete/:fontId',
    extraOptions: {},
    resourceType: 'Dashboard Font Theme',
    resourceCode: 'dashboard_font_theme__delete',
    isSuperUser: false,
    isChangeable: false,
    module: "Dashboard",
    subModule: "Font Theme"
  },
  {
    name: 'List Dashboard Themes',
    method: 'GET',
    resourceId: '/common/dashboardTheme/list',
    extraOptions: {},
    resourceType: 'Dashboard Theme',
    resourceCode: 'dashboard_theme__list',
    isSuperUser: false,
    isChangeable: false,
    module: "Dashboard",
    subModule: "Theme"
  },
  {
    name: 'Create Dashboard Theme',
    method: 'POST',
    resourceId: '/common/dashboardTheme/create',
    extraOptions: {},
    resourceType: 'Dashboard Theme',
    resourceCode: 'dashboard_theme__create',
    isSuperUser: false,
    isChangeable: false,
    module: "Dashboard",
    subModule: "Theme"
  },
  {
    name: 'Update Dashboard Theme',
    method: 'PUT',
    resourceId: '/common/dashboardTheme/update/:themeId',
    extraOptions: {},
    resourceType: 'Dashboard Theme',
    resourceCode: 'dashboard_theme__update',
    isSuperUser: false,
    isChangeable: false,
    module: "Dashboard",
    subModule: "Theme"
  },
  {
    name: 'Delete Dashboard Theme',
    method: 'DELETE',
    resourceId: '/common/dashboardTheme/delete/:themeId',
    extraOptions: {},
    resourceType: 'Dashboard Theme',
    resourceCode: 'dashboard_theme__delete',
    isSuperUser: false,
    isChangeable: false,
    module: "Dashboard",
    subModule: "Theme"
  },
  // GET current user profile image
  {
    name: 'Get Current User Profile Image',
    method: 'GET',
    resourceId: '/common/user/image',
    extraOptions: {},
    resourceType: 'User Profile Image',
    resourceCode: 'user_profile_image__get',
    isSuperUser: false,
    isChangeable: false,
    module: "Setting",
    subModule: "User Profile"
  },

  // DELETE current user profile image
  {
    name: 'Delete Current User Profile Image',
    method: 'DELETE',
    resourceId: '/common/user/image',
    extraOptions: {},
    resourceType: 'User Profile Image',
    resourceCode: 'user_profile_image__delete',
    isSuperUser: false,
    isChangeable: false,
    module: "Setting",
    subModule: "User Profile"
    
  },
  {
    name: 'Create Department',
    method: 'POST',
    resourceId: '/common/department/create',
    extraOptions: {},
    resourceType: 'Department',
    resourceCode: 'department__create',
    isSuperUser: false,
    isChangeable: true,
    module: "Setting",
    subModule: "Department"
  },
  {
    name: 'Update Department',
    method: 'PUT',
    resourceId: '/common/department/update/:departmentId',
    extraOptions: {},
    resourceType: 'Department',
    resourceCode: 'department__update',
    isSuperUser: false,
    isChangeable: true,
    module: "Setting",
    subModule: "Department"
  },
  {
    name: 'List Departments',
    method: 'GET',
    resourceId: '/common/department/list',
    extraOptions: {},
    resourceType: 'Department',
    resourceCode: 'department__list',
    isSuperUser: false,
    isChangeable: true,
    module: "Setting",
    subModule: "Department"
  },
  {
    name: 'Delete Department',
    method: 'DELETE',
    resourceId: '/common/department/delete/:departmentId',
    extraOptions: {},
    resourceType: 'Department',
    resourceCode: 'department__delete',
    isSuperUser: false,
    isChangeable: true,
    module: "Setting",
    subModule: "Department"
  },
  {
    name: 'Create Designation',
    method: 'POST',
    resourceId: '/common/designation/create',
    extraOptions: {},
    resourceType: 'Designation',
    resourceCode: 'designation__create',
    isSuperUser: false,
    isChangeable: true,
    module: "Setting",
    subModule: "Designation"
  },
  {
    name: 'Update Designation',
    method: 'PUT',
    resourceId: '/common/designation/update/:designationId',
    extraOptions: {},
    resourceType: 'Designation',
    resourceCode: 'designation__update',
    isSuperUser: false,
    isChangeable: true,
    module: "Setting",
    subModule: "Designation"
  },
  {
    name: 'List Designations',
    method: 'GET',
    resourceId: '/common/designation/list',
    extraOptions: {},
    resourceType: 'Designation',
    resourceCode: 'designation__list',
    isSuperUser: false,
    isChangeable: true,
    module: "Setting",
    subModule: "Designation"
  },
  {
    name: 'Delete Designation',
    method: 'GET',
    resourceId: '/common/designation/delete/:designationId',
    extraOptions: {},
    resourceType: 'Designation',
    resourceCode: 'designation__delete',
    isSuperUser: false,
    isChangeable: true,
    module: "Setting",
    subModule: "Designation"
  },
  {
  name: 'Create User Data Permission',
  method: 'POST',
  resourceId: '/common/user-data-permission/create',
  extraOptions: {},
  resourceType: 'User Data Permission',
  resourceCode: 'userDataPermission__create',
  isSuperUser: false,
  isChangeable: true,
  module: "Setting",
  subModule: "User Data Permission"
},
{
  name: 'Update User Data Permission',
  method: 'PUT',
  resourceId: '/common/user-data-permission/update/:id',
  extraOptions: {},
  resourceType: 'User Data Permission',
  resourceCode: 'userDataPermission__update',
  isSuperUser: false,
  isChangeable: true,
  module: "Setting",
  subModule: "User Data Permission"
},
{
  name: 'Delete User Data Permission',
  method: 'DELETE',
  resourceId: '/common/user-data-permission/delete/:id',
  extraOptions: {},
  resourceType: 'User Data Permission',
  resourceCode: 'userDataPermission__delete',
  isSuperUser: false,
  isChangeable: true,
  module: "Setting",
  subModule: "User Data Permission"
},
{
  name: 'List User Data Permissions',
  method: 'GET',
  resourceId: '/common/user-data-permission/list',
  extraOptions: {},
  resourceType: 'User Data Permission',
  resourceCode: 'userDataPermission__list',
  isSuperUser: false,
  isChangeable: true,
  module: "Setting",
  subModule: "User Data Permission"
},
{
  name: 'Get User Data Permission',
  method: 'GET',
  resourceId: '/common/user-data-permission/:id',
  extraOptions: {},
  resourceType: 'User Data Permission',
  resourceCode: 'userDataPermission__get',
  isSuperUser: false,
  isChangeable: false,
  module: "Setting",
  subModule: "User Data Permission"
},
{
    name: 'List Download Requests',
    method: 'GET',
    resourceId: '/common/download-request/list',
    extraOptions: {},
    resourceType: 'DownloadRequest',
    resourceCode: 'downloadRequest__list',
    isSuperUser: false,
    isChangeable: false,
    module: "Setting",
    subModule: "Data Export Job"
  },
  {
    name: 'Download File',
    method: 'GET',
    resourceId: '/common/download-request/download/:id',
    extraOptions: {},
    resourceType: 'DownloadRequest',
    resourceCode: 'downloadRequest__download',
    isSuperUser: false,
    isChangeable: false,
    module: "Setting",
    subModule: "Data Export Job"
  },
   {
    name: 'List Organization Visibility Settings',
    method: 'GET',
    resourceId: '/common/organization-visibility-setting/list',
    extraOptions: {},
    resourceType: 'Organization Visibility Setting',
    resourceCode: 'org_visibility__list',
    isSuperUser: false,
    isChangeable: true,
    module: "Setting",
    subModule: "Chart"
  },
  {
    name: 'Create Organization Visibility Setting',
    method: 'POST',
    resourceId: '/common/organization-visibility-setting/create',
    extraOptions: {},
    resourceType: 'Organization Visibility Setting',
    resourceCode: 'org_visibility__create',
    isSuperUser: false,
    isChangeable: true,
    module: "Setting",
    subModule: "Chart"
  },
  {
    name: 'Update Organization Visibility Setting',
    method: 'PUT',
    resourceId: '/common/organization-visibility-setting/update/:settingId',
    extraOptions: {},
    resourceType: 'Organization Visibility Setting',
    resourceCode: 'org_visibility__update',
    isSuperUser: false,
    isChangeable: true,
    module: "Setting",
    subModule: "Chart"
  },
  {
    name: 'Delete Organization Visibility Setting',
    method: 'DELETE',
    resourceId: '/common/organization-visibility-setting/delete/:settingId',
    extraOptions: {},
    resourceType: 'Organization Visibility Setting',
    resourceCode: 'org_visibility__delete',
    isSuperUser: false,
    isChangeable: true,
    module: "Setting",
    subModule: "Chart"
  },
    {
    name: 'Create Default Dashboard',
    method: 'POST',
    resourceId: '/common/role-default-dashboard/create',
    extraOptions: {},
    resourceType: 'Default Dashboard',
    resourceCode: 'default_dashboard__create',
    isSuperUser: false,
    isChangeable: true,
    module: 'Dashboard',
    subModule: 'Default Dashboard',
  },
  {
    name: 'Update Default Dashboard',
    method: 'PUT',
    resourceId: '/common/role-default-dashboard/update/:roleId',
    extraOptions: {},
    resourceType: 'Default Dashboard',
    resourceCode: 'default_dashboard__update',
    isSuperUser: false,
    isChangeable: true,
    module: 'Dashboard',
    subModule: 'Default Dashboard',
  },
  {
    name: 'List Default Dashboards',
    method: 'GET',
    resourceId: '/common/role-default-dashboard/list',
    extraOptions: {},
    resourceType: 'Default Dashboard',
    resourceCode: 'default_dashboard__list',
    isSuperUser: false,
    isChangeable: true,
    module: 'Dashboard',
    subModule: 'Default Dashboard',
  },
  {
    name: 'Delete Default Dashboard',
    method: 'DELETE',
    resourceId: '/common/role-default-dashboard/delete/:roleId',
    extraOptions: {},
    resourceType: 'Default Dashboard',
    resourceCode: 'default_dashboard__delete',
    isSuperUser: false,
    isChangeable: true,
    module: 'Dashboard',
    subModule: 'Default Dashboard',
  },
  {
    name: 'Create Business Unit',
    method: 'POST',
    resourceId: '/common/business-unit/create',
    extraOptions: {},
    resourceType: 'BusinessUnit',
    resourceCode: 'business_unit__create',
    isSuperUser: false,
    isChangeable: true,
    module: 'Setting',
    subModule: 'Business Unit',
  },
  {
    name: 'Update Business Unit',
    method: 'PUT',
    resourceId: '/common/business-unit/update/:businessUnitId',
    extraOptions: {},
    resourceType: 'BusinessUnit',
    resourceCode: 'business_unit__update',
    isSuperUser: false,
    isChangeable: true,
    module: 'Setting',
    subModule: 'Business Unit',
  },
  {
    name: 'List Business Units',
    method: 'GET',
    resourceId: '/common/business-unit/list',
    extraOptions: {},
    resourceType: 'BusinessUnit',
    resourceCode: 'business_unit__list',
    isSuperUser: false,
    isChangeable: true,
    module: 'Setting',
    subModule: 'Business Unit',
  },
  {
    name: 'Delete Business Unit',
    method: 'DELETE',
    resourceId: '/common/business-unit/delete/:businessUnitId',
    extraOptions: {},
    resourceType: 'BusinessUnit',
    resourceCode: 'business_unit__delete',
    isSuperUser: false,
    isChangeable: true,
    module: 'Setting',
    subModule: 'Business Unit',
  },
  {
    name: 'Create Vendor',
    method: 'POST',
    resourceId: '/invoicivixVendor/vendor/create',
    extraOptions: {},
    resourceType: 'Vendor',
    resourceCode: 'vendor__create',
    isSuperUser: false,
    isChangeable: true,
    module: "Setting",
    subModule: "Vendor",
  },
  {
    name: 'Update Vendor',
    method: 'PUT',
    resourceId: '/invoicivixVendor/vendor/update/:vendorId',
    extraOptions: {},
    resourceType: 'Vendor',
    resourceCode: 'vendor__update',
    isSuperUser: false,
    isChangeable: true,
    module: "Setting",
    subModule: "Vendor",
  },
  {
    name: 'Delete Vendor',
    method: 'DELETE',
    resourceId: '/invoicivixVendor/vendor/delete/:vendorId',
    extraOptions: {},
    resourceType: 'Vendor',
    resourceCode: 'vendor__delete',
    isSuperUser: false,
    isChangeable: true,
    module: "Setting",
    subModule: "Vendor",
  },
  {
    name: 'Vendor List',
    method: 'GET',
    resourceId: '/invoicivixVendor/vendor/list',
    extraOptions: {},
    resourceType: 'Vendor',
    resourceCode: 'vendor__list',
    isSuperUser: false,
    isChangeable: true,
    module: "Setting",
    subModule: "Vendor",
  },
  {
    name: 'Get Vendor',
    method: 'GET',
    resourceId: '/invoicivixVendor/vendor/:vendorId',
    extraOptions: {},
    resourceType: 'Vendor',
    resourceCode: 'vendor__get',
    isSuperUser: false,
    isChangeable: false,
    module: "Setting",
    subModule: "Vendor",
  },

  {
  name: 'Create Engagement Letter',
  method: 'POST',
  resourceId: '/invoicivixVendor/engagement-letter/create',
  extraOptions: {},
  resourceType: 'EngagementLetter',
  resourceCode: 'engagement_letter__create',
  isSuperUser: false,
  isChangeable: true,
  module: "Setting",
  subModule: "Engagement Letter",
},
{
  name: 'Update Engagement Letter',
  method: 'PUT',
  resourceId: '/invoicivixVendor/engagement-letter/update/:engagementLetterId',
  extraOptions: {},
  resourceType: 'EngagementLetter',
  resourceCode: 'engagement_letter__update',
  isSuperUser: false,
  isChangeable: true,
  module: "Setting",
  subModule: "Engagement Letter",
},
{
  name: 'Delete Engagement Letter',
  method: 'DELETE',
  resourceId: '/invoicivixVendor/engagement-letter/delete/:engagementLetterId',
  extraOptions: {},
  resourceType: 'EngagementLetter',
  resourceCode: 'engagement_letter__delete',
  isSuperUser: false,
  isChangeable: true,
  module: "Setting",
  subModule: "Engagement Letter",
},
{
  name: 'Engagement Letter List',
  method: 'GET',
  resourceId: '/invoicivixVendor/engagement-letter/list',
  extraOptions: {},
  resourceType: 'EngagementLetter',
  resourceCode: 'engagement_letter__list',
  isSuperUser: false,
  isChangeable: true,
  module: "Setting",
  subModule: "Engagement Letter",
},
{
  name: 'Get Engagement Letter',
  method: 'GET',
  resourceId: '/invoicivixVendor/engagement-letter/:engagementLetterId',
  extraOptions: {},
  resourceType: 'EngagementLetter',
  resourceCode: 'engagement_letter__get',
  isSuperUser: false,
  isChangeable: false,
  module: "Setting",
  subModule: "Engagement Letter",
},

{
  name: 'Create Legal Document',
  method: 'POST',
  resourceId: '/invoicivixVendor/legal-document/create',
  extraOptions: {},
  resourceType: 'LegalDocument',
  resourceCode: 'legal_document__create',
  isSuperUser: false,
  isChangeable: true,
  module: "Setting",
  subModule: "Legal Document",
},
{
  name: 'Update Legal Document',
  method: 'PUT',
  resourceId: '/invoicivixVendor/legal-document/update/:legalDocumentId',
  extraOptions: {},
  resourceType: 'LegalDocument',
  resourceCode: 'legal_document__update',
  isSuperUser: false,
  isChangeable: true,
  module: "Setting",
  subModule: "Legal Document",
},
{
  name: 'Delete Legal Document',
  method: 'DELETE',
  resourceId: '/invoicivixVendor/legal-document/delete/:legalDocumentId',
  extraOptions: {},
  resourceType: 'LegalDocument',
  resourceCode: 'legal_document__delete',
  isSuperUser: false,
  isChangeable: true,
  module: "Setting",
  subModule: "Legal Document",
},
{
  name: 'Legal Document List',
  method: 'GET',
  resourceId: '/invoicivixVendor/legal-document/list',
  extraOptions: {},
  resourceType: 'LegalDocument',
  resourceCode: 'legal_document__list',
  isSuperUser: false,
  isChangeable: true,
  module: "Setting",
  subModule: "Legal Document",
},
{
  name: 'Get Legal Document',
  method: 'GET',
  resourceId: '/invoicivixVendor/legal-document/:legalDocumentId',
  extraOptions: {},
  resourceType: 'LegalDocument',
  resourceCode: 'legal_document__get',
  isSuperUser: false,
  isChangeable: false,
  module: "Setting",
  subModule: "Legal Document",
},
{
  name: 'Create Activity Rate Card',
  method: 'POST',
  resourceId: '/invoicivixVendor/activity-rate-card/create',
  extraOptions: {},
  resourceType: 'ActivityRateCard',
  resourceCode: 'activity_rate_card__create',
  isSuperUser: false,
  isChangeable: true,
  module: "Setting",
  subModule: "Activity Rate Card",
},
{
  name: 'Update Activity Rate Card',
  method: 'PUT',
  resourceId: '/invoicivixVendor/activity-rate-card/update/:activityRateCardId',
  extraOptions: {},
  resourceType: 'ActivityRateCard',
  resourceCode: 'activity_rate_card__update',
  isSuperUser: false,
  isChangeable: true,
  module: "Setting",
  subModule: "Activity Rate Card",
},
{
  name: 'Delete Activity Rate Card',
  method: 'DELETE',
  resourceId: '/invoicivixVendor/activity-rate-card/delete/:activityRateCardId',
  extraOptions: {},
  resourceType: 'ActivityRateCard',
  resourceCode: 'activity_rate_card__delete',
  isSuperUser: false,
  isChangeable: true,
  module: "Setting",
  subModule: "Activity Rate Card",
},
{
  name: 'Activity Rate Card List',
  method: 'GET',
  resourceId: '/invoicivixVendor/activity-rate-card/list',
  extraOptions: {},
  resourceType: 'ActivityRateCard',
  resourceCode: 'activity_rate_card__list',
  isSuperUser: false,
  isChangeable: true,
  module: "Setting",
  subModule: "Activity Rate Card",
},
{
  name: 'Get Activity Rate Card',
  method: 'GET',
  resourceId: '/invoicivixVendor/activity-rate-card/:activityRateCardId',
  extraOptions: {},
  resourceType: 'ActivityRateCard',
  resourceCode: 'activity_rate_card__get',
  isSuperUser: false,
  isChangeable: false,
  module: "Setting",
  subModule: "Activity Rate Card",
},

{
  name: 'Create Vendor Attorney',
  method: 'POST',
  resourceId: '/invoicivixVendor/vendor-attorney/create',
  extraOptions: {},
  resourceType: 'VendorAttorney',
  resourceCode: 'vendor_attorney__create',
  isSuperUser: false,
  isChangeable: true,
  module: "Setting",
  subModule: "VendorAttorney",
},
{
  name: 'Update Vendor Attorney',
  method: 'PUT',
  resourceId: '/invoicivixVendor/vendor-attorney/update/:attorneyId',
  extraOptions: {},
  resourceType: 'VendorAttorney',
  resourceCode: 'vendor_attorney__update',
  isSuperUser: false,
  isChangeable: true,
  module: "Setting",
  subModule: "VendorAttorney",
},
{
  name: 'Delete Vendor Attorney',
  method: 'DELETE',
  resourceId: '/invoicivixVendor/vendor-attorney/delete/:attorneyId',
  extraOptions: {},
  resourceType: 'VendorAttorney',
  resourceCode: 'vendor_attorney__delete',
  isSuperUser: false,
  isChangeable: true,
  module: "Setting",
  subModule: "VendorAttorney",
},
{
  name: 'Vendor Attorney List',
  method: 'GET',
  resourceId: '/invoicivixVendor/vendor-attorney/list',
  extraOptions: {},
  resourceType: 'VendorAttorney',
  resourceCode: 'vendor_attorney__list',
  isSuperUser: false,
  isChangeable: true,
  module: "Setting",
  subModule: "VendorAttorney",
},
{
  name: 'Get Vendor Attorney',
  method: 'GET',
  resourceId: '/invoicivixVendor/vendor-attorney/:attorneyId',
  extraOptions: {},
  resourceType: 'VendorAttorney',
  resourceCode: 'vendor_attorney__get',
  isSuperUser: false,
  isChangeable: false,
  module: "Setting",
  subModule: "VendorAttorney",
},

{
  name: 'Create Vendor Invoice',
  method: 'POST',
  resourceId: '/invoicivixVendor/vendor-invoice/create',
  extraOptions: {},
  resourceType: 'VendorInvoice',
  resourceCode: 'vendor_invoice__create',
  isSuperUser: false,
  isChangeable: true,
  module: "Setting",
  subModule: "VendorInvoice",
},
{
  name: 'Update Vendor Invoice',
  method: 'PUT',
  resourceId: '/invoicivixVendor/vendor-invoice/update/:invoiceId',
  extraOptions: {},
  resourceType: 'VendorInvoice',
  resourceCode: 'vendor_invoice__update',
  isSuperUser: false,
  isChangeable: true,
  module: "Setting",
  subModule: "VendorInvoice",
},
{
  name: 'Delete Vendor Invoice',
  method: 'DELETE',
  resourceId: '/invoicivixVendor/vendor-invoice/delete/:invoiceId',
  extraOptions: {},
  resourceType: 'VendorInvoice',
  resourceCode: 'vendor_invoice__delete',
  isSuperUser: false,
  isChangeable: true,
  module: "Setting",
  subModule: "VendorInvoice",
},
{
  name: 'Vendor Invoice List',
  method: 'GET',
  resourceId: '/invoicivixVendor/vendor-invoice/list',
  extraOptions: {},
  resourceType: 'VendorInvoice',
  resourceCode: 'vendor_invoice__list',
  isSuperUser: false,
  isChangeable: true,
  module: "Setting",
  subModule: "VendorInvoice",
},
{
  name: 'Get Vendor Invoice',
  method: 'GET',
  resourceId: '/invoicivixVendor/vendor-invoice/:invoiceId',
  extraOptions: {},
  resourceType: 'VendorInvoice',
  resourceCode: 'vendor_invoice__get',
  isSuperUser: false,
  isChangeable: false,
  module: "Setting",
  subModule: "VendorInvoice",
},

{
  name: 'Create Payment Info',
  method: 'POST',
  resourceId: '/invoicivixVendor/payment-info/create',
  extraOptions: {},
  resourceType: 'PaymentInfo',
  resourceCode: 'payment_info__create',
  isSuperUser: false,
  isChangeable: true,
  module: "Setting",
  subModule: "Payment Info",
},
{
  name: 'Update Payment Info',
  method: 'PUT',
  resourceId: '/invoicivixVendor/payment-info/update/:paymentId',
  extraOptions: {},
  resourceType: 'PaymentInfo',
  resourceCode: 'payment_info__update',
  isSuperUser: false,
  isChangeable: true,
  module: "Setting",
  subModule: "Payment Info",
},
{
  name: 'Delete Payment Info',
  method: 'DELETE',
  resourceId: '/invoicivixVendor/payment-info/delete/:paymentId',
  extraOptions: {},
  resourceType: 'PaymentInfo',
  resourceCode: 'payment_info__delete',
  isSuperUser: false,
  isChangeable: true,
  module: "Setting",
  subModule: "Payment Info",
},
{
  name: 'Payment Info List',
  method: 'GET',
  resourceId: '/invoicivixVendor/payment-info/list',
  extraOptions: {},
  resourceType: 'PaymentInfo',
  resourceCode: 'payment_info__list',
  isSuperUser: false,
  isChangeable: true,
  module: "Setting",
  subModule: "Payment Info",
},
{
  name: 'Get Payment Info',
  method: 'GET',
  resourceId: '/invoicivixVendor/payment-info/:paymentId',
  extraOptions: {},
  resourceType: 'PaymentInfo',
  resourceCode: 'payment_info__get',
  isSuperUser: false,
  isChangeable: false,
  module: "Setting",
  subModule: "Payment Info",
},
{
  name: 'Payment Summary',
  method: 'GET',
  resourceId: '/invoicivixVendor/payment-info/summary',
  extraOptions: {},
  resourceType: 'PaymentInfo',
  resourceCode: 'payment_info__summary',
  isSuperUser: false,
  isChangeable: false,
  module: "Setting",
  subModule: "Payment Info",
},
{
    name: 'Create Purchase Order',
    method: 'POST',
    resourceId: '/invoicivixVendor/purchase-order/create',
    extraOptions: {},
    resourceType: 'PurchaseOrder',
    resourceCode: 'purchase_order__create',
    isSuperUser: false,
    isChangeable: true,
    module: "Setting",
    subModule: "Purchase Order",
},
{
    name: 'Update Purchase Order',
    method: 'PUT',
    resourceId: '/invoicivixVendor/purchase-order/update/:poId',
    extraOptions: {},
    resourceType: 'PurchaseOrder',
    resourceCode: 'purchase_order__update',
    isSuperUser: false,
    isChangeable: true,
    module: "Setting",
    subModule: "Purchase Order",
},
{
  name: 'Delete Purchase Order',
  method: 'DELETE',
  resourceId: '/invoicivixVendor/purchase-order/delete/:poId',
  extraOptions: {},
  resourceType: 'PurchaseOrder',
  resourceCode: 'purchase_order__delete',
  isSuperUser: false,
  isChangeable: true,
  module: "Setting",
  subModule: "Purchase Order",
},
{
  name: 'Purchase Order List',
  method: 'GET',
  resourceId: '/invoicivixVendor/purchase-order/list',
  extraOptions: {},
  resourceType: 'PurchaseOrder',
  resourceCode: 'purchase_order__list',
  isSuperUser: false,
  isChangeable: true,
  module: "Setting",
  subModule: "Purchase Order",
},
{
  name: 'Get Purchase Order',
  method: 'GET',
  resourceId: '/invoicivixVendor/purchase-order/:poId',
  extraOptions: {},
  resourceType: 'PurchaseOrder',
  resourceCode: 'purchase_order__get',
  isSuperUser: false,
  isChangeable: false,
  module: "Setting",
  subModule: "Purchase Order",
},
{
  name: 'Purchase Order Summary',
  method: 'GET',
  resourceId: '/invoicivixVendor/purchase-order/summary',
  extraOptions: {},
  resourceType: 'PurchaseOrder',
  resourceCode: 'purchase_order__summary',
  isSuperUser: false,
  isChangeable: false,
  module: "Setting",
  subModule: "Purchase Order",
},

{
  name: 'Create Activity',
  method: 'POST',
  resourceId: '/activity-info/create',
  extraOptions: {},
  resourceType: 'Activity',
  resourceCode: 'activity__create',
  isSuperUser: false,
  isChangeable: true,
  module: "Setting",
  subModule: "Activity",
},
{
  name: 'Update Activity',
  method: 'PUT',
  resourceId: '/activity-info/update/:activityId',
  extraOptions: {},
  resourceType: 'Activity',
  resourceCode: 'activity__update',
  isSuperUser: false,
  isChangeable: true,
  module: "Setting",
  subModule: "Activity",
},
{
  name: 'Delete Activity',
  method: 'DELETE',
  resourceId: '/activity-info/delete/:activityId',
  extraOptions: {},
  resourceType: 'Activity',
  resourceCode: 'activity__delete',
  isSuperUser: false,
  isChangeable: true,
  module: "Setting",
  subModule: "Activity",
},
{
  name: 'Activity List',
  method: 'GET',
  resourceId: '/activity-info/list',
  extraOptions: {},
  resourceType: 'Activity',
  resourceCode: 'activity__list',
  isSuperUser: false,
  isChangeable: true,
  module: "Setting",
  subModule: "Activity",
},
{
  name: 'Get Activity',
  method: 'GET',
  resourceId: '/activity-info/:activityId',
  extraOptions: {},
  resourceType: 'Activity',
  resourceCode: 'activity__get',
  isSuperUser: false,
  isChangeable: false,
  module: "Setting",
  subModule: "Activity",
}

];

export async function seedPermissions(permissionList: any[]) {
  const dynamicPermission = await getDynamicPermission(permissionList);
  permissions = [...permissions, ...dynamicPermission];

  const methodNameArr: Record<string, string> = {
    POST: 'create',
    PUT: 'update',
    DELETE: 'delete',
    GET: 'list',
  };

  for (const perm of permissions) {
    const { resourceCode } = perm;

    // build query
    const query: any = { resourceCode };
    if (perm.organizationId) {
      query.organizationId = perm.organizationId;
    }

    const existing = await Permission.findOne(query);

    const methodName = methodNameArr[perm.method?.toUpperCase()] || 'UNKNOWN';

    if (!existing) {
      // CREATE
      const newPermission = new Permission({
        ...perm,
        status: 'active',
        methodName,
        extraOptions: perm.extraOptions || {},
      });

      await newPermission.save();
      console.info(`✅ Permission "${perm.name}" created.`);
    } else {

  const updateData: any = {
    ...perm,
    status: existing.status || "active",
    methodName,
    extraOptions: perm.extraOptions || {},
  };

  const updatePayload: any = {};

  for (const key of Object.keys(updateData)) {
    const oldVal = existing[key];
    const newVal = updateData[key];

    const isMissing = oldVal === undefined || oldVal === null;
    const isDifferent =
      JSON.stringify(oldVal) !== JSON.stringify(newVal);

    if (isMissing || isDifferent) {
      updatePayload[key] = newVal;
    }
  }
  
  if (Object.keys(updatePayload).length > 0) {
    await Permission.updateOne(
      { _id: existing._id },
      { $set: updatePayload }
    );

    console.info(`🔄 Permission "${perm.name}" updated.`, updatePayload);
  } else {
    console.info(`ℹ️ Permission "${perm.name}" already up-to-date.`);
  }
}

  }
}


