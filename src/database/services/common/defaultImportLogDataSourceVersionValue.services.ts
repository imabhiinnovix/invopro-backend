...
export const getDataSourceImportVersionSummaryValue = async ({
  schemaName,
  query,
  select = '',
  page,
...
const vendorField = "Law Firm Name";
const vendorPath = await getReferenceField(vendorField);

groupId[vendorField] = `$${vendorPath}`;

// ✅ OPTIONAL segregation
if (segregationField) {
  const segregationPath = await getReferenceField(segregationField);
  groupId[segregationField] = `$${segregationPath}`;
}

   const groupStage: any = { _id: groupId };

   groupStage.conversion = { $first: "$conversion" };
...
for (const field of summaryFields) {
  ...
  $project: {
    ...
    conversion: 1,
    rowData: {
      [vendorField]: `$_id.${vendorField}`, // ✅ add this
      ...(segregationField
        ? { [segregationField]: `$_id.${segregationField}` }
        : {}),
      ...summaryFields.reduce((acc: any, f: string) => {
        acc[f] = `$${f}`;
        return acc;
      }, {})
    }
    ...
    const summarySort: any = {};

    if (sort && Object.keys(sort).length > 0) {
      Object.assign(summarySort, sort); // ⚠️ no rowData prefix
    } else {
...
const firstRowConversion = rawData?.[0]?.conversion || null;
...
const currencies = (facetResult.currencies || [])
  ...
  .filter((c: any) => c != null);
    ...
    // const countResult = await DataSourceVersionValue.aggregate(countPipeline).exec();
    // const totalCount = countResult?.[0]?.totalCount || 0;
    return { data: transformedData, totalCount, total: { ...grandTotalRaw, currencies, conversion: firstRowConversion } };
  } catch (err) {
    console.log('err',err);
    throw err;
  }
